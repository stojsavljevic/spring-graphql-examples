...
public class DataHandler {
	...
	ArrayList<Post> posts = new ArrayList<>(
			Arrays.asList(new Post("1", "Pet Sematary", "Television! Teacher, mother, secret lover.", 1983, "1"),
					new Post("2", "Animal Farm", "Even communism works… in theory.", 1945, "2"),
					new Post("3", "The Brothers Karamazov", "Don’t eat me. I have a wife and kids. Eat them.", 1880,
							"3"),
					new Post("4", "The Trial", "Trying is the first step towards failure", 1925, "4"),
					new Post("5", "Murder on the Orient Express",
							"To alcohol! The cause of, and solution to, all of life’s problems.", 1934, "5")));

	ArrayList<Author> authors = new ArrayList<>(Arrays.asList(new Author("1", "Stephen King", "king@gmail.com"),
	...
	public Post createPost(PostInput postInput) {
		...
		Post post = new Post(String.valueOf(this.posts.size() + 1), postInput.getTitle(), postInput.getContent(),
				Year.now().getValue(), postInput.getAuthorId());
		this.posts.add(post);
		return post;
	}
	...
	public Post getPostById(String postId) {
		Optional<Post> post = this.posts.stream().filter(p -> p.getId().equals(postId)).findAny();
		return post.get();
	}
